export default {
  header: {
    title: "Orlando Language School",
    message: "A melhor escola de inglês em Orlando",
    icon: require("../../assets/header/brazil-flag.png"),
    language_picker_helper: "PT",
    languages: {
      english: "EN",
      spanish: "ES",
      portuguese: "PT",
    },
    home: "INÍCIO",
    programs: "PROGRAMAS",
    visas: "VISTOS",
    galleries: "GALERIAS",
    about: "QUEM SOMOS",
    contact: "CONTATE-NOS",

    enrollment: "INSCRIÇÃO",
    mainMessage: "Aprendendo inglês, facilitado!",
    button: "VERIFIQUE NOSSOS PROGRAMAS AQUI!",
  },
  join: {
    title: "Participe do nosso programa de tempo integral!",
    parrafo: `<span class =" font-weight-bold bold - text "> Vistos de estudante F1 </span> e I-20s para <span class =" font-weight-bold primary--text "> estudantes internacionais </span> <br> <span class = "font-weight-bold gray--text"> Inscreva-se </span> com o nosso <span class = "font-weight-bold gray--text"> passo a passo guia de etapas </span> Comece <span class = "font-weight-bold primary--text"> estudando inglês em Orlando </span> hoje! `,
    parrafo2: "Informações sobre o programa de tempo integral aqui",
  },
  discover: {
    title1: "PROGRAMA EM TEMPO INTEGRAL",
    p1: `O programa F1 Visa para estudantes internacionais permite que você estude nos Estados Unidos em período integral. É um programa intensivo onde você aprenderá e aprimorará suas habilidades em inglês. Oferecemos um horário flexível para estudantes a preços acessíveis e, de fato, os melhores da região de Orlando.
    <br><br>
    Não se estresse: forneceremos um <router-link to="/step-by-step">guia passo a passo</router-link> fácil sobre como solicitar seu visto de estudante F1!`,
    title2: "MUDANÇA DE ESTADO",
    p2: `
    Siga este simples guia passo a passo para solicitar sua alteração de status ou entre em contato conosco para que possamos fornecer todas as informações necessárias. Simples e fácil. Feito isso, emitiremos toda a sua documentação e informaremos quando começar.   <br><br>
     A Orlando Language School está autorizada pelo SEVP a matricular estudantes estrangeiros não imigrantes que desejam estudar inglês em Orlando.`,
    title3: "PROGRAMA DE TEMPO PARCIAL",
    p3: `Este programa é focado e adaptado para estudantes que desejam completar curtos períodos de férias ou visitando temporariamente. Também disponível para cidadãos e residentes legais dos Estados Unidos.
    <br>   <br>
    <ul> <li>
    O programa intensivo ajuda você a melhorar suas habilidades em inglês </li> <li>
    Horário flexível para todos os alunos </li> <li>
    Os melhores preços da região de Orlando </li> </ul>

    `,
    button: "SAIBA MAIS",
    headline: "Descubra o seu programa de inglês perfeito",
  },
  aboutUs: {
    title1: "Sobre nós",
    title2: "Nossa missão",
    p2:
      "É fornecer a melhor experiência de aprendizado de inglês disponível para promover o rápido entendimento e uso, empregando programas e materiais dinâmicos e centrados no aluno para a aquisição de um segundo idioma, além de manter uma equipe de professores de ESL bem treinados e altamente motivados, O objetivo é ajudar seus alunos a ganhar confiança e a falar inglês.",
  },
  followUs: "Siga-nos nas redes sociais:",

  footer: {
    home: "Casa",
    programs: "Programas",
    about: "Quem Somos",
    course: "Cursos e Objetivos",
    contact: "Entre em contato",
    emailus: "Escreva-nos",
    galleries: "Galerias",

    enrollment: "inscrição",
    callus: "Ligue-nos",
  },
  programs: {
    achievement: "Escala de desempenho e interpretação dos níveis",
    fulltime: "Programa a tempo inteiro",
    ftP1: `A Orlando Language School oferece programação intensiva para estudantes internacionais de visto F1 em tempo integral, variando de iniciantes a universitários. Nosso principal programa de inglês é baseado em 10 níveis, ministrados pelos instrutores mais qualificados. Níveis avançados estão disponíveis após a conclusão do nosso programa principal. Também oferecemos cursos preparatórios para o TOEFL.`,
    ftTitle1: `CERTIFICAÇÕES DE RESCISÃO`,
    ftP2: `Todos os alunos que concluem o programa recebem um certificado de participação no final de seus estudos, mediante solicitação. Todos os alunos são avaliados continuamente e um relatório de avaliação está disponível no final de seus estudos. `,
    ftTitle2: `PREÇO`,
    ftPricingList: `<ul>
    <li>$530 USD a cada quatro semanas</li>
    <li>18 horas de aula por semana: um total de 72 horas de aula em 28 dias</li>
    <li><a href="https://www.ols4u.com/policiesFT.pdf">Política escolar de período integral</a></li>
    <li><a href="https://www.ols4u.com/enrollmentFT.pdf"> Estudantes de contrato de inscrição em período integral (FT) e I-20</a></li>
    </ul>`,
    ftTitle3: `ENSAIO DE COLOCAÇÃO`,
    ftP3: `Para todos os nossos programas, oferecemos um teste de nível gratuito para todos os alunos que chegam. De preferência, é tomado um dia antes do início do período. <br> <br> Os livros didáticos e as taxas de inscrição não se aplicam se você estiver matriculado durante o ano.`,
    fTScheduleTitle: "CRONOGRAMA DE TEMPO INTEGRAL",
    ftMorning: "MANHÃ",
    ftMorningP: "Segunda a quinta-feira das 9:00 às 13:45",
    ftAfternoon: "TARDE",
    ftAfternoonP:
      "De segunda a quinta-feira, das 14h às 17h45 | sexta-feira, das 12h45 às 17h",
    ftEvening: "NOITE",
    ftEveningP: "Segunda a quinta-feira das 18:00 às 22h45",

    parttime: "Programas de meio período",
    ptP1: `Para todos os nossos programas, oferecemos um teste de colocação gratuito para todos os alunos que chegam. Abaixo, listamos os recursos e características do nosso programa de meio período. Este programa é focado e personalizado para estudantes que desejam concluir curtos períodos de férias, visitando temporariamente ou cidadãos e residentes legais dos Estados Unidos da América`,

    ptTitle2: `PROGRAMA SUPER INTENSIVO - PARTE TEMPO`,
    ptPricingList: `<ul>
    <li>$ 485 USD a cada quatro semanas</li>
    <li>16,75 horas de aula por semana - 67 horas de aula em 28 dias</li>
    <li>Livros de gramática e conversas incluídos - 1 conjunto por nível </li>
    <li>Taxa de registro única de US $ 50 USD</li>
    <li><a href="https://www.ols4u.com/policiesPT.pdf"">Política escolar do programa de meio período</a></li>
    <li><a href="https://www.ols4u.com/enrollmentPT.pdf">Contrato de inscrição Alunos a tempo parcial (PT) e formulário de inscrição</a></li>
    </ul>`,

    ptScheduleTitle: "CRONOGRAMA DE PROGRAMAS DE TEMPO PARCIAL",
    ptMorning: "MANHÃ",
    ptMorningP:
      "De segunda a quarta-feira, das 9:00 às 13:45 h | Quinta-feira, das 9:00 às 12:30 h",
    ptAfternoon: "TARDE",
    ptAfternoonP:
      "De segunda a quinta-feira, das 14h às 17h45 | sextas-feiras das 12h45 às 17h",
    ptEvening: "NOITE",
    ptEveningP:
      "De segunda a quarta-feira, das 18:00 às 22:00 h | quintas-feiras das 18:00 às 21:30 h",
    ipgTitle1: "GRAMÁTICA INTENSIVA DO PROGRAMA (IGP)",
    ipgPricingList: `<ul>
    <li>$ 325 USD a cada quatro semanas</li>
    <li>6.75 horas de aula por semana - total de 27 horas de aula em 28 dias</li>
    <li>Livros de gramática incluídos - 1 conjunto por nível </li>
    <li>Taxa de registro única de US $ 50 USD</li>
    </ul>`,
    ipgTitle2: "CRONOGRAMA DO IGP",
    ipgMorning: "MANHÃ",
    ipgMorningP: "Segunda a quinta-feira das 9:00 às 11:30",
    ipgAfternoon: "TARDE",
    ipgAfternoonP:
      "Segunda-feira - quinta-feira, das 14:00 às 16:00 horas - sextas-feiras das 12:45 às 14:45",
    ipgEvening: "NOITE",
    ipgEveningP: "Segunda a quinta-feira das 18:00 às 20:30",

    icpTitle1: "PROGRAMA DE CONVERSAÇÃO INTENSIVO (ICP)",
    icpPricingList: `<ul>
    <li>$ 230 USD a cada quatro semanas</li>
    <li>6.75 horas de aula por semana - total de 27 horas de aula em 28 dias</li>
    <li>Livros de conversação incluídos - 1 conjunto por nível </li>
    <li>Taxa de registro única de US $ 50 USD</li>
    </ul>`,
    icpTitle2: "CRONOGRAMA DO IPC",
    icpMorning: "MANHÃ",
    icpMorningP:
      "Segunda a quarta, das 11:45 às 13:45 <br>Quinta-feira, das 11h45 às 12h30",
    icpAfternoon: "TARDE",
    icpAfternoonP:
      "Segunda a quinta-feira, das 16h15 às 17h45 <br> Sextas-feiras das 15:00 às 15:45",
    icpEvening: "NOITE",
    icpEveningP:
      "Monday through Wednesday from 8:45 pm to 10:45 pm <br>Thursday from 8:45 pm to 9:30 pm",
    privateTitle: "AULAS PRIVADAS",
    privateList: `<ul>
    <li><b>
Aulas ministradas na escola.</b><br>US $ 45 por hora de aula de manhã, tarde e / ou noite</li>
        <li><b>Aulas ministradas na escola - Pacote Especial</b><br>US $ 35 por hora de aula ao reservar pacotes com mais de 20 horas de manhã, tarde e / ou noite</li>
            <li><b>Instalações do aluno</b><br>US $ 50 por hora de aula e 0,40 centavos de dólar por milha adicionais nas instalações do aluno de manhã, tarde e / ou noite</li>
    </ul> 
    Taxa de registro única de US $ 50 USD`,
    enroll: [
      {
        registrationDates: "6 de janeiro a 13 de março de 2020",
        startDate: "23 de março de 2020",
        endDate: "29 de maio de 2020",
      },
      {
        registrationDates: "6 de janeiro a 29 de junho de 2020",
        startDate: "1 de junho de 2020",
        endDate: "7 de agosto de 2020",
      },
      {
        registrationDates: "6 de janeiro a 7 de agosto de 2020",
        startDate: "10 de agosto de 2020",
        endDate: "9 de outubro de 2020",
      },
      {
        registrationDates: "6 de janeiro a 9 de outubro de 2020",
        startDate: "13 de outubro de 2020",
        endDate: "18 de dezembro de 2020",
      },
    ],
    enrollment: {
      enrollmentTitle: "Datas de inscrição para 2020",
      term: "Term",
      registrationDates: "Datas de registro",
      startDate: "Data de início",
      endDate: "Data final",
    },
    structure: {
      mainTitle: "Estrutura e objetivos",
      title1: "ESTRUTURA DO CURSO",
      p1: `Somos especialistas no ensino de inglês como segunda língua. Nosso programa de ESL é baseado em 10 níveis ditados por instrutores profissionais qualificados. Níveis mais avançados estão disponíveis após a conclusão do nosso programa principal.
<br> <br>
Os novos alunos são obrigados a fazer um teste de nivelamento para avaliar sua proficiência em inglês. Este teste consiste em 70 perguntas distribuídas em três seções: audição, leitura e uso da linguagem. Os alunos têm 50 minutos para concluir este teste. `,
      title2: "OBJETIVOS DO CURSO",
      p2:
        "Existem várias maneiras pelas quais os alunos podem tirar proveito de seus cursos na Orlando Language School. Seguindo os próximos objetivos, seu entendimento, pronúncia e gramática do inglês melhorarão exponencialmente, dando a você a confiança de um falante nativo de inglês.",
      title3: "GRAMATICALMENTE E CONVERSACIONALMENTE",
      p3: `O OLS oferece cursos nos níveis de 1 a 10 (iniciantes a avançados).Uma variedade de níveis estendidos inclui um suporte instrucional mais alto para estudantes universitários, preparação para exames ou crescimento profissional.Esses cursos incluem:
        <br>
          <ul> <li> Interação individual e em grupo </li>
            <li> Pronúncia</li>
            <li> Apresentação oral </li>
            <li> E comunicação profissional, que inclui escrever e falar </li> </ul>`,
    },
    curriculum: {
      mainTitle: "Currículo",
      title1: "NÍVEIS DE PROFICIÊNCIA EM LÍNGUA",
      p1: `A filosofia acadêmica da Orlando Language School é aprender através da comunicação. Para que os alunos aprendam rapidamente e progridam no programa, uma unidade é concluída toda semana em todos os nossos aulas de gramática e conversação, que incluem os seguintes materiais e atividades do curso.<br> <br> Existem 10 níveis padrão de instrução em nosso sistema, além de nosso programa de nível estendido. Os níveis 1 a 8 levam 9 semanas para serem concluídos. Os níveis 9 e 10 e os níveis estendidos levam 13 semanas para serem concluídos. <br> <br> Os níveis de proficiência em idiomas da OLS permitem que os alunos progridam a um ritmo consistente com a prática de um idioma recém-adquirido.<br> <br> Conversas educacionais, atividades de trabalho em grupo, jogos e ocasionais excursões extracurriculares permitem que os alunos interajam com diferentes culturas e origens de todo o mundo, aprimorando a experiência de aprendizado que oferecemos.`,
      title2: "GRAMÁTICA",
      p2: ` <ul> <li> Níveis 1 a 8: Intercâmbio de classes gramaticais (8 unidades por nível) </li> <li> Níveis 9 a 10: Classes de gramática de passagens (12 unidades por nível) </li> <li> Níveis estendidos 1 e 2: preparação para o TOEFL (5 unidades e 4 unidades por nível) </li> <li> Nível 3 e 4 estendido: foco gramatical (11 unidades e 12 unidades por nível) </li> </ul> `,
      title3: "CONVERSAÇÃO",
      p3: ` <ul> <li> Níveis 1 - 2: Decolando Classes de Conversação (6 unidades por nível)</li>
        <liNíveis 3 a 8: vamos conversar sobre aulas de conversação (8 unidades por nível) </li>
      <li> Níveis 9 - 10: Mosaico 1 e 2 - Aulas de conversação e conversação (10 capítulos por nível) </li>
      <li> Níveis estendidos 1 e 2: aulas de conversação de leitura do mosaico 1 e 2 (10 capítulos por nível) </li>
      <li> Níveis estendidos 3 e 4: aulas de conversação de escrita do mosaico 1 e 2 (10 capítulos por nível) </li> </ul>`,
    },
  },
  visas: {
    mainTitle: "Guia passo a passo para solicitação de visto F1",
    stepByStep: `
<ol class="olClass">

    <li>
        Entre em contato com o programa em que você está interessado e com a data de início preferida para iniciar o processo de inscrição na Orlando Language School.
    </li>

    <li>
        Deixe-nos saber quem você é:
        <ul> <b>
        <li> Você está se inscrevendo no seu país de origem?
        </li> <li> Você está se transferindo de outra escola?
        </li> <li> Você está solicitando uma mudança de status?
        </li> </b>
        </ul>
        </li>

              <li>
                  Forneça os documentos necessários para todos os candidatos I-20.
                  <ul>
                  
                      <li>
                          Preencha o contrato de inscrição na Orlando Language School e nossa Política escolar </li>
                  
                      <li> Envie-nos uma cópia clara da sua página de identificação com foto do passaporte </li>
                  
                      <li> Prova de suporte financeiro, como uma carta bancária original em papel timbrado ou uma cópia do extrato bancário mais recente, demonstrando fundos disponíveis suficientes para pagar suas mensalidades, taxas e custos de vida para você e quaisquer dependentes durante sua estadia nos EUA
                          < / li>
                          
                              <li> <b> Se um patrocinador está financiando financeiramente, precisamos de uma carta de patrocinador original com firma reconhecida </b>, uma carta bancária original em papel timbrado ou uma cópia do extrato bancário mais recente, demonstrando fundos disponíveis suficientes para pagar sua taxa de matrícula , taxas e custos de vida para você e quaisquer dependentes durante a sua estadia nos EUA </li>
                          
                              <li> Envie-os para info@ols4u.com </li>
                          
                              <li> <b> Se você estiver se transferindo de outra escola, </b> envie-nos uma cópia clara do seu formulário I-20 mais recente, uma cópia clara do seu visto atual de dentro do seu passaporte e uma cópia clara do seu Formulário I-94 ou carimbo do porto de entrada (POE) no seu passaporte. Ou uma cópia do aviso de aprovação (I-797) de qualquer alteração de status concedida </li>
                          
                              <li> <b> Se você está solicitando uma mudança de status, </b> envie-nos uma cópia clara do seu visto atual de dentro do seu passaporte e uma cópia clara de ambos os lados do seu carimbo do Formulário I-94 ou POE no seu passaporte. </li>
                          
                              <ul>
                              
                                  <li>
                                      A Orlando Language School está enviando a solicitação de COS em seu nome, incluindo também:
                                      <ul>
                                      
                                          <li>
                                              Taxa de inscrição I-539 de US $ 370 (USD) pagável por cheque pessoal ou ordem de pagamento ao Departamento de Segurança Interna dos Estados Unidos </li>
                                      
                                          <li> Aplicativo I-539 preenchido e assinado original </li>
                                          </ul>
                                      </li>
                                  </ul>
                              </ul>
                  </li>
              
                  <li>
                      Enviaremos ou forneceremos o Formulário I-20. Você precisará levar o Formulário I-20 com você para sua entrevista de visto e apresentá-lo ao funcionário consular ou da embaixada. Antes de participar da sua entrevista de visto, você precisará pagar sua taxa I-901 SEVIS. <a href="https://www.fmjfee.com/i901fee/index.html"> Você pode visitar um site oficial do Departamento de Segurança Interna </a> para saber mais sobre essa taxa e efetuar o pagamento.
                  </li>
              
                  <li>
                      Encontre uma embaixada ou consulado dos EUA, geralmente em seu país de origem, onde você pode agendar e participar de sua entrevista de visto.
                  </li>
              
                  <li>
                      Antes de sua entrevista, você deve preencher o pedido de visto online. Este formulário de inscrição é chamado Formulário DS-160. Você pode ler mais sobre o pedido de visto e as taxas visitando o site oficial do Departamento de Estado
                  </li>
              
                  <li> Saiba antes de ir! Leia os pontos a seguir com atenção e colete todos os documentos de suporte. Você deve levar documentação de suporte completa e concisa ao compromisso. Este pacote suporta as informações exigidas no pedido de visto e tem grande influência na aprovação ou negação final do seu pedido.
                      <br>Os documentos de suporte incluem:
                      <br>
                      <ul>
                          <li>
                              O Formulário I-20 certificado, emitido pela Orlando Language School
                          </li>
                          <li>
                              Confirmação de pagamento SEVIS I-901
                          </li>
                          <li>
                              Se aplicável, documentação de vínculos com seu país de origem
                          </li>
                          <li>
                              Documentos de identificação originais, incluindo: passaporte válido, certidão de nascimento, carteira de motorista, duas fotos recentes em estilo de passaporte
                          </li>
                          </ul>
                      </li>
          </ol>`,
    contact: `Esperamos que você tenha achado nosso guia passo a passo útil <br>
       Entre em contato conosco para tirar dúvidas adicionais! `,
  },
  about: {
    mainTitle: "Quem Somos",
    ourMission: `<span class="title"> Nossa missão ... </span><br> <br>Nossa missão é fornecer a melhor experiência de aprendizado de inglês disponível para promover rápida compreensão e uso, empregando programas, materiais e metodologia dinâmicos, centrados no aluno, para aquisição de segundo idioma, além de manter uma equipe de funcionários bem treinados, altamente motivados, Professores de ESL cujo objetivo é ajudar seus alunos a ganhar confiança e a ter sucesso ao falar inglês. `,
    title1: "A escola",
    p1:
      "Comemorando mais de 25 anos de ensino de inglês, a Orlando Language School ocupa o primeiro nível na área de Orlando por nossa experiência, metodologia e instalações de alta tecnologia que nos permitem ensinar inglês como segunda língua.",
    p2:
      "'Estou estudando lá e estou melhorando a cada dia. Os professores são bons. A escola é ótima e todos que trabalham lá são muito úteis. Obrigado escola de idiomas da OLS pelo ótimo trabalho! '",
    firma: "JULIANA PICCINATO",
    title2: "Corpo docente e pessoal",
    p3:
      "As altas credenciais acadêmicas da equipe e a ampla experiência em educação nos ajudam a selecionar nossos instrutores nativos. Sua dedicação, determinação e paixão pelo ensino refletem no progresso das habilidades de fala de nossos alunos",
  },

  contact: {
    t1: "Entre em contato",
    p1:
      "Basta preencher o formulário abaixo e entraremos em contato com você o mais rápido possível.",
    l1: "nome",
    l2: "telefone",
    l3: "E-mail",
    l4: "Informações adicionais",
    button1: "Enviar",
    t2: "Detalhes de contato",
    button2: "Voltar",
  },
  galleries: {
    mainTitle: "Ano novo, novo visual!",
    subMainTitle:
      "VISITE A NOSSA INSTALAÇÃO RECENTEMENTE REMODELADA COM ESPAÇOSO ESPAÇO E MODERNO PARA ESTUDANTES!",
    title1: "NOVO visual e nova tecnologia!",
    p1:
      "Também temos o prazer de anunciar a parcela dos SMARTBOARDS SHARP AQUOS 4K de 70 polegadas em TODAS as nossas salas de aula. Estes são os smartboards de última geração com aplicativos de quadro branco integrados e conectividade à Internet. Os professores da OLS agora podem apresentar atividades de ESL e descrições gramaticais digitalmente no smartboard usando a mais recente tecnologia de smartboard e nossos alunos podem interagir diretamente com as placas usando seus telefones celulares! ",
    title2:
      "Aproveite os últimos dias de nossa promoção. SEM TAXAS, MAIS 20% DE DESCONTO EM TUITION - Apresse-se !!!",
    buttonFull: "Programa completo",
    contact: "Entre em contato",
    p2: `Venha visitar nossas instalações RECENTEMENTE REMODELADAS com uma sala de estudantes espaçosa e luxuosa!
Temos o prazer de anunciar a instalação de placas inteligentes SHARP AQUOS 4K de 70 polegadas em TODAS as nossas salas de aula.Esses são os painéis inteligentes de última geração com aplicativos de quadro branco integrados e conectividade à Internet.Os professores do OLS agora podem apresentar atividades de ESL e descrições gramaticais digitalmente no usando a mais recente tecnologia de smart board e nossos alunos podem interagir diretamente com as tabelas usando seus telefones celulares! `,
    testimonies1: `“ Melhor custo-benefício de Orlando. Ótima localização e excelentes profissionais. Eu estudo lá há quase um ano e estou adorando. ”<br>
<span class = "title"> - CAMILA DIAS </span>
`,
    testimonies2: `“ Esta foi a melhor experiência da minha vida. Ser professor na OLS tem sido incrível, conhecer meus alunos, suas culturas e fazer parte de sua família. ”<br>
<span class = "title"> - DIANA GIRALDO </span>
`,
    testimonies3: `“ Minha experiência foi a melhor! Ótimos professores e o Diretor Acadêmico sempre acompanhando sua evolução. Todo o suporte sempre muito rápido e profissional! ”<br>
<span class = "title"> - VANDERLEI LELLIS </span>
`,
    testimonies4: `“ Os professores são fabulosos e surpreendentes, com sotaques e culturas diferentes. Perfeito para os alunos ... Quero dizer, esta é sua melhor escolha para aprender inglês em Orlando. ”<br>
<span class = "title"> - AGUINALDO CUPERTINO </span>
`,
    fieldTitle: "Viagens de campo!",
    fieldP:
      "Trabalhe duro, jogue duro. Na OLS, você pode visitar locais emblemáticos em Orlando com seus colegas de classe!",
    celebrationsTitle: "Celebrações!",
    celebrationsP:
      "Seja no Dia das Bruxas, Ação de Graças ou Natal, na OLS, gostamos de comemorar tudo!",
  },
  landing: {
    t1: "Você ousa aprender inglês?",
    t2:
      "Junte-se ao nosso seleto grupo de estudantes aqui em Orlando FL, cadastre-se e comece a trabalhar com você e seu futuro",
    t3:
      "Ols4u, Orlando, FL, com mais de 25 anos de experiência, professores de alto nível, o melhor método e uma infraestrutura moderna, tudo à sua disposição",
    t4: "Junte-se a nós",
    p1: "POR QUE ESCOLA DE LÍNGUA ORLANDO?",
    p2: "Como nosso método é 100% garantido e se adapta a você.",
    p3:
      "Nossa missão é ajudá-lo a falar o idioma inglês com segurança e em pouco tempo.",
    p4: "Aprendendo para você:",
    p5:
      "Nós fornecemos a melhor experiência de aprendizado através do uso de programas, materiais e metodologia dinâmicos, centrados no aluno. Recentemente, investimos para oferecer a você uma das mais modernas infraestruturas do sul da Flórida",
    p6: "Experiência:",
    p7:
      "Temos mais de 25 anos de experiência e uma equipe de professores e instrutores de alto nível. Eles garantirão o sucesso de sua adaptação ao nosso método. Nossos professores seguem uma abordagem comunicativa e centrada no aluno para o ensino e aprendizagem de idiomas. Inglês ",
    p8: "Acreditações:",
    p9:
      "O OLs4U está autorizado pelo Programa SEVP para Estudantes e Visitantes de Intercâmbio a matricular estudantes estrangeiros não imigrantes que desejam estudar inglês em Orlando.",
    p10:
      "O Ols4u possui o credenciamento concedido pela Comissão de Credenciamento do Programa de Língua Inglesa (CEA), é a única agência de credenciamento reconhecida pela Secretaria de Educação dos Estados Unidos. Seu objetivo é promover a excelência no campo do ensino e da administração. Programas de inglês com a ajuda de processos de credenciamento em todo o mundo. ",
    p11:
      "Não perca esta oportunidade! Inscreva-se dessa maneira e aproveite um desconto especial de 40%",
    p12: "PART TIME",
    p122: "GRAMÁTICA<br> (IGP)",
    p1222: "CONVERSAÇÃO<br> (ICP)",
    p13: "$ 485 -40% OFF",
    p133: "$ 325 -40% OFF",
    p1333: "$ 230 -40% OFF",
    p14: "$ 291",
    p144: "$ 195",
    p1444: "$ 138",
    p15: "16,75 horas de aula por semana",
    p155: "6,75 horas de aula por semana",
    p1555: "6,75 horas de aula por semana",
    p16: "67,5 horas de aula por mês",
    p166: "27 horas de aula por mês",
    p1666: "27 horas de aula por mês",
    p17: "Livros de gramática e conversação incluídos: 1 jogo por nível",
    p177: "Livros de gramática incluídos - 1 conjunto por nível",
    p1777: "Livros de conversação incluídos - 1 conjunto por nível",
    p18: "Horário flexível",
    p19: "Taxa por 4 semanas",
    p20:
      "* Dependendo do caso e do perfil do aluno, a administração do OLS4U pode aplicar uma única taxa adicional de US $ 50, no momento da inscrição e entrega dos recibos",
    p112: "TEMPO INTEIRO",
    p113: "desconto de US $530 -40",
    p114: "$318",
    p115: "18 horas de aula por semana",
    p116: "72 horas de aula por mês",
    p117: "Livros de gramática e conversação incluídos: 1 jogo por nível",
    p118: "Horário flexível",
    p119: "Taxa por 4 semanas",
    btn: "Inscreva-se hoje",
  },
  success: {
    t1: "Seu pagamento foi concluído com êxito",
    t2: "Entraremos em contato em breve por e-mail.",
    t3: "Ir para casa",
  },
  rejected: {
    t1: "Não foi possível processar seu pagamento com êxito",
    t2: "tente novamente",
    t3: "Tente novamente",
  },

  visaf1: "Atende aos requisitos de visto F-1",
  popup1: "Aprenda inglês online de qualquer lugar do mundo!",
  popup2: "PROGRAMAS ONLINE",
  underline: "Agora disponível online - estude em qualquer lugar do mundo!",
};
